...
\title{mCRL2 exercises -- Vending machines}
...
\subex{
Produce the labelled transition system (LTS) of this specification using (1) \code{mcrl22lps} to linearise the system and (2) \code{lps2lts} to produce the final LTS.}

\subex{Visualise the previous LTS with \code{ltsgraph}.
\textbf{Show a screenshot of LTS (after rearranging as needed.}}
...
\subex{Linearise your specification, build the LTS, and visualise the result LTS using \code{mcrl22lps}, \code{lps2lts}, and \code{ltsgraph}.
...
\begin{exercise}  
Recall the \texttt{vm2.mcrl2} specification from Exercise~\ref{ex:vm2} of a system performing \code{coin50} and \code{coin100} actions as well as so-called $\tau$-steps.

\subex{Build a specification \texttt{vm2-taus.mcrl2} that is the same as \texttt{vm2.mcrl2} after hiding the actions \code{readyA}, \code{readyC}, \code{out50}, \code{out100}, \code{prod}. \textbf{Show this specification.}}

\subex{Using the \code{ltscompare} tool, compare your model under branching bisimilarity with the LTS of the system \code{vm2-taus} with the LTS of the system \texttt{vm2} after hiding the actions \code{readyA}, \code{readyC}, \code{out50}, \code{out100}, \code{prod}. For that use the following command.}
...
\subex{Using \code{ltsconvert}, minimise the LTS for \texttt{vm2.mcrl2} with respect to branching bisimulation after hiding the same actions as before (using the \code{hide} operation in mcrl2).
\textbf{Visualise and include a screenshot of the minimised LTS.}
Compare the minimised LTSs and \texttt{vm2-taus.lts} visually using \code{ltsgraph}.}
...
\begin{enumerate}
\item use ``\texttt{lps2pbes <lps-file> -f <file-with-formula> <output.pbes>}'' to produce a system of boolean equations, and
\item use ``\texttt{pbes2bool <output.pbes>}'' to evaluate if these equations have a solution.  
\end{enumerate}
}
