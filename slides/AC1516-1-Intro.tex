...
\item \dkb{run-time structures}: such as \dkb{object instances}, \dkb{clients}, \dkb{servers}, \dkb{databases},  \dkb{browsers}, \dkb{channels}, \dkb{broadcasters}, \dkb{software buses}, ...
...
\item \rdb{Background}
\begin{itemize}
\item \dkb{(Semantics) Introduction to transition systems} 
  \dgold{\textsf{mCRL2}}
\item \dkb{(Logic) Introduction to modal, hybrid and dynamic logic}
\end{itemize}
...
\item \dkb{Method:} The \dgold{\textsf{mMm}} calculus; prototyping in  \textsc{Haskell} 
\end{itemize}
