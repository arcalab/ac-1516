...
\input{./macros/LSBslides-mac.tex}
...
\item l\dgold{ogical consequence}: $\Phi \models \phi$ ($\phi$ is satisfied in every model of $\Phi$)
...
\begin{itemize}
\item Propositional logic (logic of \dgold{uninterpreted assertions}; models are \dgold{truth assignments})
\item Equational logic (formalises \dgold{equational} reasoning; models are \dgold{algebras})
\item First-order logic (logic of \dgold{predicates} and \dgold{quatification} over structures; models are \dgold{relational structures})
\item Modal logics 
\item ...
\end{itemize}
...
The \dgold{signature} of the basic modal language is determined by sets $\mathsf{PROP}$ of \dgold{propositional} symbols (typically assumed to be denumerably infinite) and $\mathsf{MOD}$ of \dgold{modality} symbols.
\end{slide}
...
A \dkb{model} for the language is a pair $\ger{M} = \pair{\blk{F},V}$, where
\begin{itemize}
\item $\ger{F} = \pair{W, \enset{R_m}_{m \in \mathsf{MOD}}}$  \\ is a \dkb{Kripke frame}, ie, a non empty set $W$ and a family of binary relations
over $W$, one for each modality symbol $m \in \mathsf{MOD}$. Elements of $W$ are called \dgold{points},  \dgold{states},  \dgold{worlds} or
...
Normal modal logics are \dgold{axiomatic extensions to $\mathbf{K}$}
...
\item $P, 0 \models \always \eventual \always false\; \;$  iff  ... %every element is below  a maximal element 
\end{itemize}
\end{block}
\end{slide}
...
\noindent
For any such structure, a new modality, $\nexts$, can be defined based on the \dgold{cover} relation $\lessdot$ for $<$ (\ie, the smallest relation whose transitive closure is $<$). Thus,
\begin{align*}
...
\begin{itemize}
\item \dgold{linear}: $\rcb{\forall}{x,y}{x,y \in T}{x=y \wedge x<y \wedge y<x}$.
\item \dgold{discrete}: for each $t\in T$, i) if there is a $u>t$ there is a first such $u$; ii) if there is a $u<t$ there is a last such $u$.
\item \dgold{dense}: if for all $t,x \in T$, if $x<t$ there is a $v\in T$ such that $x<v<t$.
\item \dgold{Dedekind complete}: if for all $S\subseteq T$ non-empty and bounded above, there is a lest upper bound in $T$.
...
\small
Boxes and diamonds are essentially a \dgold{macro notation} to encode quantification over accessible states in a point free way. \\
\vspace{0.2cm}
...
\begin{block}{Express the following properties in Process Logic}
\begin{itemize}
\item \dgold{inevitability of $a$}: \dkb{$\pv{-}{\true} \e \nc{-a}{\false}$}
\item \dgold{progress}: \dkb{$\pv{-}{\true}$}
\item \dgold{deadlock or termination}: \dkb{$\nc{-}{\false}$}
\item what about
\begin{equation*}
\dkb{\pv{-}{\false}}\; \; \; \text{and}\; \; \;  \dkb{\nc{-}{\true}}\; \; \; \text{?}
\end{equation*}
...
Given two models $\ger{M} = \pair{\pair{W, R}, V}$ and $\ger{M}' = \pair{\pair{W', R'}, V'}$, and a \dgold{bisimulation} 
...
& & & \text{and for all}\; u\; \text{st}\; u R w\; \text{and}\; v R u,\; \text{one has}\;  \ger{M}, u \models \phi\\
\ger{M}, w  &\models \phi\, \mathcal{S}\, \psi & \text{ ~~ \textsf{iff} ~~} &
\text{there exists}\; v \in W \;\text{st}\; w R v\; \text{and}\; \ger{M}, v \models \psi,\\
& & & \text{and for all}\; u\; \text{st}\; u R v\; \text{and}\; w R u,\; \text{one has}\;  \ger{M}, u \models \phi
\end{align*}
\end{block}
%
\begin{itemize}
...
%
%----------------------------------------------------------------------------------
\begin{slide}{\red Exercise}\label{s:42}
\small
...
\small
 ...
 \dkb{state} formulas to express  properties of a state:
\[
...
\end{equation*}
for $i \in \mathsf{NOM}$ (a \dgold{nominal})
\end{block}
\end{slide}
...
\end{block}
\begin{flushright}
\dgold{[recent research at HASLab: projects \textsc{Mondrian} and \textsc{Nasoni}]}
\end{flushright}
\end{slide}
