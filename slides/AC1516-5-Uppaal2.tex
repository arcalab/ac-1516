...
\begin{block}{Timed bisimulation (between temporal LTS)}
...
Additionally,
\begin{itemize}
\item $ta.\ell$ which tests \dgold{current location}:  $(\ell, \eta) \models ta.\ell$ \\
provided $(\ell, \eta)$ is a state in $\TL{ta}$
\item $\texttt{deadlock}$: $(\ell, \eta) \models \forall_{d \in \R_0^+} .\, \text{there is no transition from} \; \pair{\ell,\eta+d}$ 
\end{itemize}
\end{block}
...
\\[2mm]
\Psi\; ::=\; & ta.\ell ~|~ g_c ~|~ g_d ~|~ \texttt{not~}\Psi ~|~ \Psi \texttt{ or }\Psi ~|~ \Psi \texttt{ and }\Psi ~|~ \Psi \texttt{ imply }\Psi
\end{align*}
